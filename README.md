...
实现 Modelica 语言与信号语义的广覆盖编译能力：解析、展平、BLT（块下三角）排序、JIT（默认）与可选 AOT 编译及仿真。路线以与 OpenModelica 行为对齐为目标（回归与对标流程见 `docs/regression/README.md` 与 `REQUIREMENTS_ANALYSIS_REPORT.md`），并持续向全量信号覆盖推进。支持 FMI 2.0 CS/ME 导出（工具链与脚本说明见 `jit-compiler/docs/regression/toolchain-fmi-emitc-script.md`）。该引擎被 ModAI 在运行时调用，其能力边界通过 ModAI 内自迭代持续扩展。
...
- **范围界定**：以“尽量实现全量信号覆盖”为目标，按阶段推进与 OMC 的行为一致性；当前版本仍存在未覆盖项并在持续收敛。MSL / ModelicaTest 回归与目录说明见 `jit-compiler/docs/regression/msl-modelicatest-dir-regression.md`，并逐步扩展。
- **安全与可维护性**：AI 生成补丁在沙箱中执行；API Key 加密存储；大文件拆分规范（800 行阈值）详见 `OPTIMIZATION_PLAN.md`。
...
> - 能力范围与回归矩阵以仓库内 `docs/regression/`、`jit-compiler/docs/regression/` 与当前 CI/脚本为准（不再引用已移除的独立任务清单文件名）。
...
| 回归体系 | 仓库根 `run_regression.ps1`、`run_modelica_dir_regression.ps1`；`cargo run -p regress-harness -- jit …`（含 `validate-perf` / `compare-baseline` 等）；`compare_omc.ps1`；CI 与 `docs/regression/` | [x] 主矩阵与 JIT perf 基线流程已接入；默认 `jit compare-baseline` 基线见 `baseline/README.md` 与 `baseline/20260418_three_tier_devloop/jit_perf_baseline.json` | [x] |
...
- **前端**：基于 Pest 的语法解析器、AST、加载器及展平模块（继承、实例化、connect、数组、for/when）。流水线与优化计划详见 `OPTIMIZATION_PLAN.md`。
...
- **运行与导出**：`--result-file` CSV、`--repl`、`--script`、`--emit-c`、`--emit-fmu`、`--emit-fmu-me`（FMI 2.0 CS/ME）。详见 `jit-compiler/docs/regression/toolchain-fmi-emitc-script.md`。
...
- **MSL 支持**：固定子集（Constants、SIunits、Blocks 子集、内置 Math）。目录回归与约束见 `jit-compiler/docs/regression/msl-modelicatest-dir-regression.md`。
...
- **AI 功能**：自然语言生成 Modelica 代码、AI 面板；JIT 遇限时提示“使用 AI 补全”并触发自迭代：描述目标 → 生成/粘贴 diff → 沙箱（check、build/test、mo 回归）→ 采纳或提交。产品侧说明见 `modai-ide/README.md`。
...
- **当前状态**：核心编译/仿真、JIT/AOT、FMI 2.0 CS/ME 导出、IDE 基础功能及自迭代流程已实现，已建立“全量信号覆盖”推进主线，并持续扩展回归与 DIR/MSL 覆盖面（见 `docs/regression/README.md`）。
- **MVP / 近期**：P0 编辑 + AI 生成、JIT 验证 + 仿真、可视化；P1 编译器功能补全、2–3 轮自迭代与性能提升（在 ModAI 内完成，与 JIT 能力扩展形成闭环）；P2 跨平台稳定与 token 预算控制。需求与范围可参考 `REQUIREMENTS_ANALYSIS_REPORT.md`、`modai-ide/README.md`。
- **后续方向**：扩展 MSL/语法覆盖、FMI 导入、更多求解器与运行时选项；差距与计划类材料见 `LEYDEN_GAP_ANALYSIS.md`、`OPTIMIZATION_PLAN.md`、`CHANGELOG.md`。

---

## 项目结构

- **Workspace**（根目录 `Cargo.toml`）：成员包括 `jit-compiler`（包名 `rustmodlica`，CLI 与库）、`modai-ide/src-tauri`、`crates/regress-harness`、`crates/modai-paths`、`crates/modai-protocol`、`modai-worker`。运行时 ModAI 调用 JIT，自迭代则修改 jit-compiler 源码并重新构建，二者在运行时相互促进。
...
- **FMI 导出**：`--emit-fmu=<dir>`（CS）、`--emit-fmu-me=<dir>`（ME）。详见 `jit-compiler/docs/regression/toolchain-fmi-emitc-script.md`。
...
- Newton 稀疏求解器配置：
  ...
  - 稀疏路径验证：使用 `RUSTMODLICA_NEWTON_PATH_TRACE=1` 观察日志，或对应用例走 `jit-compiler/scripts/run_mos_regression.ps1`（含 `omc_regression_newton_symbolic_sparse.mos` 等）。
...
| 分类           | 文档 |
|----------------|------|
| 编译器/规范     | `docs/regression/README.md`、`jit-compiler/docs/regression/README.md`、`jit-compiler/docs/regression/toolchain-fmi-emitc-script.md`、`jit-compiler/docs/regression/msl-modelicatest-dir-regression.md`、`REQUIREMENTS_ANALYSIS_REPORT.md` |
| IDE/产品       | `modai-ide/README.md` |
| 优化/实现      | `OPTIMIZATION_PLAN.md`、`OPTIMIZATION_IMPLEMENTATION_STATUS.md`、`CHANGELOG.md`、`LEYDEN_GAP_ANALYSIS.md` |
