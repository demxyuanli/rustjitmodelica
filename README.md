...
**小结：**在对齐任务列表（P1/P2/P3）范围内，rustmodlica JIT 与 OMC 在“是否支持”的层面基本一致。
...
| 结果对比 | 小集合模型与 OMC 数值结果对比脚本 | [x] `compare_omc.ps1`（REG-2） | [x] |
...
|------|--------|-----------------|--------------|
| 同步语义 | clock / `sample` / `interval` / clocked partition | [ ] 完整支持；仅解析 + 拒绝或极简实现，SYNC-* 未全做 | [x] 完整同步语义 |
| clocked 变量 | clocked state、clock partition、connect 推断 | [ ] 仅有占位/文档，未实现真正 clock partition | [x] |
| 复杂函数 | 深递归、大型函数、不纯函数（side-effect） | [ ] 全覆盖；有 JIT stub + 深度限制，side-effect 多为阻止/报错 | [x] 更宽松、成熟 |
| 外部函数 ABI | 数组/字符串、跨平台 ABI | [x] 数组 ptr+size 已文档+部分实现；[ ] 字符串和复杂类型完整支持 | [x] 成熟且广泛使用 |
| 脚本语言 | 完整 Modelica 脚本（`.mos`） | [ ] 完整；有简化脚本/部分 OMC 风格命令（SCRIPT-*） | [x] 完整 `.mos` |
| 稀疏求解 | 稀疏 Jacobian + 稀疏线性求解接入 JIT | [ ] JIT 中真正使用 sparse；IR/CSR 已有但 Newton 仍用 dense | [x] 在大规模模型上成熟使用 |
...
| 优先级 | 方向 | 建议任务 |
|--------|------|----------|
| 高 | 同步语义 | 按 SYNC-* 依次实现 clock AST、clocked 变量、`sample`/`interval` 的时钟语义，并增加与 OMC 的对比用例。 |
| 中 | 稀疏与数值 | 将 IR 中已有的稀疏 Jacobian / CSR 求解器真正接入 JIT 的 Newton/tearing pipeline，并对大模型进行 OMC 数值/性能对比。 |
| 中 | 外部函数与脚本 | 完善 external 函数的字符串/复杂类型 ABI 实现，扩展脚本命令集以覆盖常见 `.mos` 场景，并选择若干 OMC 脚本作为回归基准。 |
