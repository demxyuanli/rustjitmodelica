...
use crate::analysis::{expression_is_zero, partial_derivative};
...
fn collect_vars_eq_local(eq: &Equation, vars: &mut HashSet<String>) {
    match eq {
        ...
        Equation::CallStmt(expr) => {
            collect_vars_expr_local(expr, vars);
        }
...
pub fn print_backend_dae_info(
    ...
    println!(
        "{}",
        i18n::msg("number_of_variables", &[&total_declarations])
    );
    ...
    if let Some(dae) = simulation_dae {
        ...
        println!(
            "{}",
            i18n::msg("states_x", &[&dae.dae.variables.state_count()])
        );
        println!(
            "{}",
            i18n::msg("derivatives", &[&dae.dae.variables.derivative_count()])
        );
        println!(
            "{}",
            i18n::msg("algebraic_z", &[&dae.dae.variables.algebraic_count()])
        );
        println!(
            "{}",
            i18n::msg("inputs_u", &[&dae.dae.variables.input_count()])
        );
        println!(
            "{}",
            i18n::msg("discrete", &[&dae.dae.variables.discrete_count()])
        );
        println!(
            "{}",
            i18n::msg("parameters_count", &[&dae.dae.variables.parameter_count()])
        );
        println!(
            "{}",
            i18n::msg("simulation_equations", &[&dae.dae.total_equation_count])
        );
        println!(
            "{}",
            i18n::msg("initial_equations", &[&dae.initial.equation_count])
        );
        if dae.dae.differential_index > 1 {
            println!(
                ...
                i18n::msg(
                    ...
                    &[&dae.dae.constraint_equation_count]
                )
            );
        }
        let (single_b, torn_b, mixed_b) =
            dae.dae
                .blocks
                .iter()
                .fold((0usize, 0usize, 0usize), |(s, t, m), b| {
                    match b.block_type {
        ...
        println!(
            ...
            i18n::msg(
                "blocks_partitioning",
                &[
                    &single_b as &dyn std::fmt::Display,
                    &torn_b as &dyn std::fmt::Display,
                    &mixed_b as &dyn std::fmt::Display
                ]
            )
        );
    }
    ...
    println!(
        "{}",
        i18n::msg("number_of_states", &[&state_vars.len(), &states_list])
    );
    println!(
        "{}",
        i18n::msg(
            "number_of_discrete",
            &[&discrete_vars.len(), &discrete_list]
        )
    );
    println!(
        "{}",
        i18n::msg(
            "clocked_states",
            &[&clocked_state_names.len(), &clocked_list]
        )
    );
    if let Some(dae) = simulation_dae {
        if !dae.dae.clock_partitions.is_empty() {
            println!(
                "{}",
                i18n::msg("clock_partitions_count", &[&dae.dae.clock_partitions.len()])
            );
    ...
    println!(
        "{}",
        i18n::msg("tearing_variables_selected", &[&tearing_var_count])
    );
    println!(
        "{}",
        i18n::msg("torn_unknowns_total", &[&torn_unknowns_total])
    );
    if !block_sizes.is_empty() {
        ...
        println!(
            "{}",
            i18n::msg("block_unknowns_min_max", &[&min_block, &max_block])
        );
    }
    if !block_residual_sizes.is_empty() {
        ...
        println!(
            "{}",
            i18n::msg("block_residuals_min_max", &[&min_res, &max_res])
        );
    }
    if counted_vars > 0 {
        ...
        println!(
            ...
            i18n::msg(
                ...
                &[
                    &min_eq_count,
                    &max_eq_count,
                    &avg_fmt as &dyn std::fmt::Display
                ]
            )
        );
    }
    println!(
        "{}",
        i18n::msg(
            "generate_dynamic_jacobian",
            &[&opts.generate_dynamic_jacobian]
        )
    );
    let scj = if strong_component_jacobians {
        i18n::msg0("yes")
    } else {
        i18n::msg0("no")
    };
    println!("{}", i18n::msg("strong_component_jacobians", &[&scj]));
    let soj = if symbolic_ode_jacobian {
        i18n::msg0("yes")
    } else {
        i18n::msg0("no")
    };
    println!("{}", i18n::msg("symbolic_ode_jacobian", &[&soj]));
    let noj = if numeric_ode_jacobian {
        i18n::msg0("yes")
    } else {
        i18n::msg0("no")
    };
    ...
    if let Some(jac) = ode_jacobian_symbolic {
        println!(
            "{}",
            i18n::msg("symbolic_jacobian_size", &[&jac.len(), &state_vars.len()])
        );
    ...
    if let Some(sparse) = ode_jacobian_sparse {
        println!(
            "{}",
            i18n::msg(
                "ode_jacobian_sparsity",
                &[
                    &sparse.nnz(),
                    &(sparse.n * sparse.n),
                    &format!("{:.2}%", sparse.density() * 100.0)
                ]
            )
        );
    }
