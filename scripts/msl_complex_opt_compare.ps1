param(
    [int]$Rounds = 5,
    [string]$ModelList = "build_regression_logs/msl_complex_candidates.txt",
    [string]$OutRoot = "build_regression_logs/msl_complex_opt_compare",
    [double]$TEnd = 5.0,
    [double]$Dt = 0.01,
    [string]$Solver = "rk4",
    [ValidateSet("0", "1")]
    [string]$AdaptiveFoldPolicyOnOpt = "0"
)
...
if (-not (Test-Path -LiteralPath $ModelList)) {
    throw "Model list not found: $ModelList"
}
...
for ($i = 1; $i -le $Rounds; $i++) {
    $optDir = "build_modelica_dir_regress_msl_complex_opt_r$i"
    $noDir = "build_modelica_dir_regress_msl_complex_noopt_r$i"

    $env:RUSTMODLICA_JIT_CODEGEN_CACHE = "0"
    $env:RUSTMODLICA_CONST_FOLD = "1"
    $env:RUSTMODLICA_EQ_DCE = "1"
    $env:RUSTMODLICA_ADAPTIVE_FOLD_POLICY = $AdaptiveFoldPolicyOnOpt
    ...
    powershell -NoProfile -File run_modelica_dir_regression.ps1 `
        -OutDir $optDir -OnlySkipsFromSummary $ModelList -TEnd $TEnd -Dt $Dt -Solver $Solver | Out-Null
    if ($LASTEXITCODE -ne 0) { throw "optimized run failed at round $i" }

    $env:RUSTMODLICA_JIT_CODEGEN_CACHE = "0"
    $env:RUSTMODLICA_CONST_FOLD = "0"
    $env:RUSTMODLICA_EQ_DCE = "0"
    $env:RUSTMODLICA_ADAPTIVE_FOLD_POLICY = "0"
    $env:RUSTMODLICA_TIERED_COMPILATION = "0"
    $env:RUSTMODLICA_WARMUP_ENABLED = "0"
    $env:RUSTMODLICA_CACHE_SQLITE = "0"
    $env:RUSTMODLICA_FLATTEN_FULL_CACHE = "0"
    $env:RUSTMODLICA_QUERY_CACHE = "0"
    powershell -NoProfile -File run_modelica_dir_regression.ps1 `
        -OutDir $noDir -OnlySkipsFromSummary $ModelList -TEnd $TEnd -Dt $Dt -Solver $Solver | Out-Null
    if ($LASTEXITCODE -ne 0) { throw "noopt run failed at round $i" }
    ...
    foreach ($r in $optRows) {
        $optAll.Add([pscustomobject]@{ round = $i; model = $r.case_name; ms = [double]$r.duration_ms }) | Out-Null
    }
    foreach ($r in $noRows) {
        $noAll.Add([pscustomobject]@{ round = $i; model = $r.case_name; ms = [double]$r.duration_ms }) | Out-Null
    }
    ...
    Write-Host ("round {0}: opt={1} noopt={2}" -f $i, $optTotal, $noTotal)
}
...
$modelStats = foreach ($m in $models) {
    ...
    [pscustomobject]@{
        ...
        speedup_mean_pct = if ($nMean -gt 0) { [math]::Round((($nMean - $oMean) / $nMean) * 100, 2) } else { 0.0 }
    }
}
...
$totalStats = [pscustomobject]@{
    rounds                = $Rounds
    adaptive_fold_policy_on_opt = $AdaptiveFoldPolicyOnOpt
    ...
    total_speedup_mean_pct = if ($ntMean -gt 0) { [math]::Round((($ntMean - $otMean) / $ntMean) * 100, 2) } else { 0.0 }
}
...
[pscustomobject]@{ total_stats = $totalStats; model_stats = $modelStats } |
ConvertTo-Json -Depth 6 | Set-Content -Encoding UTF8 (Join-Path $OutRoot "summary.json")

Write-Host "=== TOTAL ==="
$totalStats | Format-List
Write-Host "=== MODEL ==="
$modelStats | Format-Table -AutoSize
