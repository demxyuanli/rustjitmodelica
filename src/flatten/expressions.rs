...
pub fn prefix_expression(expr: &Expression, prefix: &str) -> Expression {
    let prefix_str = if prefix.is_empty() { String::new() } else { format!("{}_", prefix) };
    ...
    match expr {
        ...
        Expression::Call(func, args) => Expression::Call(
            func.clone(), 
            args.iter().map(|arg| prefix_expression(arg, prefix)).collect()
        ),
        Expression::Der(arg) => {
            Expression::Der(Box::new(prefix_expression(arg, prefix)))
        }
        Expression::ArrayAccess(arr, idx) => {
             ...
             } else if let (Expression::ArrayLiteral(elements), Expression::Number(n)) = (&arr_flat, &idx_flat) {
                 let idx = *n as usize;
                 if idx > 0 && idx <= elements.len() {
                     elements[idx - 1].clone()
                 } else {
                     eprintln!("Index out of bounds in flattening: {} (len {})", idx, elements.len());
...
pub fn index_expression(expr: &Expression, idx: usize) -> Expression {
    match expr {
        Expression::Variable(_name) => {
            Expression::ArrayAccess(Box::new(expr.clone()), Box::new(Expression::Number(idx as f64)))
        }
        ...
        Expression::ArrayLiteral(elements) => {
            if idx > 0 && idx <= elements.len() {
                ...
                eprintln!("Index out of bounds for ArrayLiteral: {} (len {})", idx, elements.len());
                Expression::Number(0.0)
            }
        }
        Expression::Call(func, args) => {
            Expression::Call(func.clone(), args.iter().map(|arg| index_expression(arg, idx)).collect())
        }
        Expression::Der(arg) => {
            Expression::Der(Box::new(index_expression(arg, idx)))
        }
