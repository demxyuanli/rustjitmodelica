...
### H1 深挖的起点：代数环求解（下次专门会话用）

用一组最小模型建立护栏（均带一个状态 `der(x)=-x` 以触发仿真），当前行为：

| 模型 | 结构 | 当前输出 | 期望 | 判定 |
|------|------|---------|------|------|
| G1 | 正向链 `a=5;b=a+1;c=b+1` | a=5,b=6,c=7 | 同 | ✅ |
| G2 | 逆序声明链 `c=b+1;b=a+1;a=5` | a=5,b=6,c=7 | 同 | ✅ |
| G3 | 耦合环 `a=2*b+1;b=3-a` | **a=1,b=2** | a=2.33,b=0.67 | ❌ |
| G4 | 残差+链 `a=b+1;b=c+1;0=a-10` | **a=10,b=1,c=0** | a=10,b=9,c=8 | ❌ |

**关键发现**：G3（最基本的 2×2 线性代数环）也错。`a=1,b=2` 恰是**顺序单遍代入**的结果（b 初值 0 → a=2·0+1=1 → b=3-1=2，未迭代）。即**代数环没有被当作块迭代求解到收敛**。

**结论**：G4 的错误不只是 H1（SCC 发射顺序）——根因在更底层的**代数环/块求解**（环未被识别为需 Newton 迭代的块，或块求解退化成单遍代入）。改 SCC 顺序治不了 G3。下次应从 G3 这个最小环入手：确认 BLT 是否把 `{a,b}` 识别为 `SolvableBlock`、块求解为何退化成单遍。

G1–G4 复现命令见本会话记录（`--solver=rk4 --t-end=0.05 --dt=0.05`，需 `RUSTMODLICA_BACKEND_DAE_CACHE=0` 等禁缓存以避免旧分析结果）。
