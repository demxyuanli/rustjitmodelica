...
**仍开放(salsa query-db 层,`--validate` 默认 `SALSA=1`)**:
**根因(系统性,已深挖确认)**:salsa 查询是**逐查询记忆化**的。手工文件依赖(`dep_record_file` → DepScope)只在查询**函数体实际执行**时记录;一旦某查询在本进程内被记忆化,下游消费者拿到的是记忆化值,其 `dep_record_file`/`dep_record_deps` **不会再注入下游的 DepScope**。因此负依赖无法可靠地沿 `source_text → parsed_items → model_ast → inheritance_flattened → decl_expanded → flattened_model_q → analyze` 逐层传播——是否到达 `--validate` 命中的下游缓存**取决于查询执行顺序**。

试过两处定点(source_text 记负依赖;inheritance_flattened 在基类缺失处 `dep_record_file` 负依赖)——`inheritance_flattened` 自身条目能拿到负依赖,但 `--validate` 命中的是**下游缓存**(decl_expanded/flattened_model_q),仍陈旧。要真正修复需**每一层查询在记忆化命中时也重新传播其完整传递依赖**,与 salsa 记忆化语义冲突,属**架构级**改动(与审查最初"H4 架构性、重"一致)。`SALSA=0`(管线路径)已正确,是可用的规避。

另注:`extends <缺失类>` 会 flaky 段错误(退化模型 codegen 崩溃),与本缓存修复无关,是既有独立 bug。
