...
负依赖已记录并持久化(`parsed_items` 条目含 `ABSENT_DEP_SENTINEL`),但**未传递到 `--validate` 命中的 `inheritance_flattened`/下游缓存条目的 deps**,故 `--validate` 默认路径仍陈旧(`SALSA=0` 正确)。属 salsa 查询依赖图的**传递性负依赖传播**问题,需专门处理:确保上游查询(inheritance/flat_model)的 dep 集包含其依赖的 `source_text`/`parsed_items` 的负依赖。
