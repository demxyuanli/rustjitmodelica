...
| 单 `.mo` 多顶层定义 | [x] | [x] | [x] | [x] | 支持同文件 `function + model`，加载器可同文件回退索引 |
...
当前状态：算法多赋值已支持数组字面量/固定数组变量分解写回，且函数多输出调用式 `(a,b,...):=f(x)` 主路径已补齐（`TwoOutputs`/`MultiOutputFunc` 已完成定向验证与回归记录）；同时已收紧异常 arity/非法 `LHS` 路径为显式错误，并补齐 `ArrayAccess` 写集分析。复杂输出形态专项样例（`MultiOutputNestedExpr`/`MultiOutputMixedArrayScalar`/`MultiOutputShapeMismatch`）已纳入主回归闭环；并新增深层形状分级（`1D array -> scalar` 为 warning，`record|multidim|comprehension -> scalar` 为 hard error）及扩展回归样例（`MultiOutputRecordShapeMismatch`/`MultiOutput2DArrayShapeMismatch`/`MultiOutputComprehensionShapeMismatch`/`MultiOutputRecordNestedArrayMismatch`/`MultiOutputCrossLayerComprehensionMismatch`/`MultiOutputComplexLhsFieldStore`/`DeepRecordNestedMismatch`/`MixedNestedLhsFieldStoreMismatch`/`CrossModuleComprehensionMismatch`/`CrossModuleRecordCompositeMismatch`/`AliasChainTypeMismatch`/`MixedMultiTargetSafePass`/`MixedMultiTargetFieldStoreFail`）。此外已将类型别名解链结果接入 `FunctionOutputSpec`，用于 MultiAssign 形状判定，并在复杂 LHS 诊断中输出索引化目标（`#n=<lhs>`）。
...
| **稀疏 Jacobian** | [x] 持续增强 | CSR 表示与稀疏求解链路可用，triples 构建支持越界过滤与重复项合并 |
...
| **外部函数 ABI** | [x] 部分 | 标量参数已支持；数组签名识别已增强（含 `ArrayLiteral`）；字符串参数仍受限 |
...
| **可用**（工作正常，范围有限） | 隐式求解器、外部函数 ABI（标量 + 部分数组）、脚本模式、REPL |
...
以下统计以当次回归快照为准；后续以最新 `run_regression` / 目录回归日志与汇总报告为准。
...
| `compare_omc.ps1` | [x] OpenModelica 数值结果对比（严格模式鲁棒：对象化返回、空值/缺失属性安全过滤、噪声输出隔离） |
...
说明：上述为一次执行快照，后续请以最新结构化日志（NDJSON/CSV）与汇总输出为准。
...
| 领域 | 差距 | 影响 | 优先级 |
|------|------|------|--------|
| JIT 中的算法多赋值 | 已支持数组字面量/固定数组变量分解写回；函数多输出调用式主路径已补齐；异常 arity/非法 LHS 已收紧；复杂输出专项样例已接入回归，并已形成 warning/hard-error 的更深形状分级扩展闭环（含更深 record 层级、mixed nested LHS、跨模块推导式、跨模块 record 复合推导、跨包类型别名链、mixed 多目标对称样例）；剩余为主回归在 Windows release 文件占用场景下的稳定性收敛 | 低（范围可控） | 低 |
| 外部函数数组/字符串 ABI | 仅支持标量参数；数组 ptr+size 和字符串已文档化但未实现 | 中（限制外部库集成） | 中 |
| 完整 `.mos` 脚本语言 | 覆盖率 98%+，仍非完整 OMC `.mos` 全集 | 低（IDE 补偿） | 低 |
| JIT Newton 中的稀疏 Jacobian | 稀疏链路已可用；默认策略仍以密集优先 | 中（大型系统的性能） | 中 |
...
| 维度 | RustModlica 优势 |
|------|-----------------|
| **编译速度** | 在当前样例与回归工况下表现较快（以最新日志汇总为准） |
...
| 低 | 算法多赋值边界增强 | 在已完成更深扩展闭环并补齐跨模块 record 复合推导、跨包类型别名链与 mixed 多目标对称样例的基础上，继续收敛 Windows release 文件占用导致的主回归 pass 统计扰动 |
...
注：热力图为阶段性快照，用于趋势展示；对外门禁以最新回归/对标日志为准。
